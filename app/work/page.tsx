        ...
        <main className="min-h-screen">
            ...
            <section className="pt-40 pb-24 px-8 md:px-12 lg:px-24">
                <div className="max-w-[1400px] mx-auto">
                    ...
                    <div className="grid grid-cols-1 gap-px bg-black border-2 border-black overflow-hidden rounded-[3rem]">
                        {cases.map((c, i) => (
                            <Reveal key={i} delay={0.4 + i * 0.2}>
                                <div className="bg-white p-16 lg:p-32 flex flex-col lg:flex-row justify-between items-start lg:items-center group hover:bg-black hover:text-white transition-all duration-700 cursor-pointer">
                                    <div className="space-y-8">
                                        <div className="text-[12px] font-black uppercase tracking-widest text-black/40 group-hover:text-white/40">{c.client}</div>
                                        <h2 className="text-5xl md:text-7xl font-black tracking-tighter">{c.project}</h2>
                                        <p className="text-2xl font-bold text-black/80 group-hover:text-white/80 max-w-2xl leading-tight">{c.impact}</p>
                                    </div>
                                    <div className="mt-16 lg:mt-0 flex flex-col items-end gap-16">
                                        <ArrowUpRight className="w-12 h-12 transform group-hover:translate-x-4 group-hover:-translate-y-4 transition-transform duration-500" />
                                        <div className="flex gap-4">
                                            {c.tags.map((tag, j) => (
                                                <span key={j} className="px-4 py-1 border border-black/10 rounded-full text-[10px] font-bold uppercase tracking-widest group-hover:border-white/20">{tag}</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
