                    ...
                    <div className="grid grid-cols-1 gap-12 border-t-2 border-black pt-24">
                        {products.map((p, i) => (
                            <Reveal key={i} delay={0.4 + i * 0.2}>
                                <div className="bg-black/5 rounded-[3rem] p-16 lg:p-32 flex flex-col lg:flex-row gap-24 group hover:bg-black hover:text-white transition-all duration-700 cursor-pointer">
                                    <div className="w-full lg:w-1/3 space-y-8">
                                        <div className="px-6 py-2 border-2 border-black/10 rounded-full inline-block text-[12px] font-black tracking-widest uppercase group-hover:border-white/20">
                                            Status: {p.status}
                                        </div>
                                        <h2 className="text-5xl md:text-6xl font-black tracking-tighter">{p.name}</h2>
                                        <div className="w-24 h-2 bg-black group-hover:bg-white transition-all" />
                                    </div>

                                    <div className="w-full lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-16">
                                        <div className="space-y-4">
                                            <div className="text-[12px] font-black uppercase tracking-widest text-black/40 group-hover:text-white/40">Problem</div>
                                            <p className="text-2xl font-bold leading-tight">{p.problem}</p>
                                        </div>
                                        <div className="space-y-4">
                                            <div className="text-[12px] font-black uppercase tracking-widest text-black/40 group-hover:text-white/40">Target Audience</div>
                                            <p className="text-2xl font-bold leading-tight">{p.audience}</p>
                                        </div>
                                        <div className="space-y-4 md:col-span-2">
                                            <div className="text-[12px] font-black uppercase tracking-widest text-black/40 group-hover:text-white/40">Vision</div>
